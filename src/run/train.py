...
def train_step(
    ...
    for i, data in enumerate(pbar):
        ...
        w_dict = None # {n_combiation_of_category_idx(i.e. ('hat', 'sunglass')): (n_subspace)}
        
        batched_qs_embs = None # (batch_sz, max_length, n_subspace, d_embed)
        batched_a_embs = None # (batch_sz, n_subspace, d_embed)
        
        batched_qs_c = None # List of (batch_sz, max_length)
        batched_a_c = None # List of (batch_sz,)
        batched_combs_of_c= [[(q_c, a_c) for q_c in qs_c] for qs_c, a_c in zip(batched_qs_c, batched_a_c)]
  
        w = [[w_dict[comb_of_c] for comb_of_c in combs_of_c] for combs_of_c in batched_combs_of_c]
        w = torch.tensor(w, device=rank) # (batch_sz, max_length, n_subspace)
        
        batched_qs_emb = batched_qs_embs * w # (batch_sz, max_length, n_subspace, d_embed)
        batched_qs_emb = batched_qs_emb.sum(dim=2) # (batch_sz, max_length, d_embed)
        
        
        n_neg = len(batched_a_embs)
        batched_cs_embs = [[batched_a_embs[j] for j in range(n_neg)] for i in range(n_neg)] # (batch_sz, n_neg, n_subspace, d_embed)
        batched_cs_embs = torch.tensor(batched_cs_embs, device=rank) # (batch_sz, n_neg, n_subspace, d_embed)
        batched_cs_embs = batched_cs_embs.unsqueeze(2) # (batch_sz, n_neg, 1, n_subspace, d_embed)
        
        batched_cs_emb = batched_cs_embs * w.unsqueeze(1) # (batch_sz, n_neg, max_length, n_subspace, d_embed)
        batched_cs_emb = batched_cs_emb.sum(dim=3) # (batch_sz, n_neg, max_length, d_embed)
        
        batched_qs_emb = batched_qs_emb.unsqueeze(1) # (batch_sz, 1, max_length, d_embed)
